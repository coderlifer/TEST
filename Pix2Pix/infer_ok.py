...
def load_examples(raw_input):
    ...
    paths_batch, inputs_batch, targets_batch = tf.train.batch(["", input_images, target_images],
...
def infer(images):
    ...
    # images_original = tf.placeholder(tf.uint8, [None, None, 3], name="input_images")
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.mode == "test":
            ...
            for step in range(max_steps):
                results = sess.run(converted_outputs)
                results = np.asarray(results[0])
