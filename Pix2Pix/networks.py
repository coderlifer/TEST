...
def resnet_g_1(generator_inputs, generator_outputs_channels, ngf, conv_type, channel_multiplier, padding,
    ...
    with tf.variable_scope("layer_1"):
        ...
        # output = norm_layer(output, decay=0.9, epsilon=1e-5, is_training=True, norm_type="IN")
        # output = nonlinearity(output, 'lrelu', 0.2)
        print('resnet_g.layer_1.output: {}'.format(output.shape.as_list()))
    ...
    layer_specs = [
        ...
        ngf * 16,  # encoder_5: [batch, 32, 32, ngf * 8] => [batch, 16, 16, ngf * 16]
    ...
    for out_channels in layer_specs:
        with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
            ...
            output = lib.ops.conv2d.Conv2D(
                output, output.shape.as_list()[-1], out_channels, 4, 2, 'Conv2D',
    ...
    print('resnet_g.layer_7.output: {}'.format(layers[-1].shape.as_list()))

    # [batch, 4, 4, ngf * 16] ----> [batch, 512, 512, ngf]
    n_block = 7
    for i in range(n_block):
        with tf.variable_scope('layer_{}'.format(len(layers) + 1)):
            out_c = ngf * min(16, 2 ** (n_block - i - 1))
            output = ResidualBlock(
                layers[-1], layers[-1].shape.as_list()[-1], out_c, 3, name='G.Block.%d' % (len(layers) + 1),
            ...
            layers.append(output)
            print('G.Block: {}'.format(layers[-1].shape.as_list()))
    ...
    with tf.variable_scope('layer_{}'.format(len(layers) + 1)):
        ...
        output = lib.ops.conv2d.Conv2D(
            output, inputs.shape.as_list()[-1], generator_outputs_channels, 5, 1, 'Conv2D',
            conv_type='conv2d', channel_multiplier=0,
            padding='VALID', spectral_normed=True, update_collection=None,
            inputs_norm=False, he_init=True, mask_type=None, weightnorm=None, biases=True, gain=1.)
        # output = norm_layer(output, decay=0.9, epsilon=1e-5, is_training=True, norm_type="IN")
...
def resnet_d_1(discrim_inputs, discrim_targets, ndf, spectral_normed, update_collection,
    ...
    # layer_3: [batch, 128, 128, ndf * 2] => [batch, 64, 64, ndf * 4]
    # layer_4: [batch, 64, 64, ndf * 4] => [batch, 32, 32, ndf * 8]
    # layer_4: [batch, 32, 32, ndf * 8] => [batch, 16, 16, ndf * 4]
    # layer_4: [batch, 16, 16, ndf * 4] => [batch, 8, 8, ndf * 2]
    layer_specs = [
        ndf * 2,  # encoder_2: [batch, 256, 256, ngf] => [batch, 128, 128, ngf * 2]
        ndf * 4,  # encoder_3: [batch, 128, 128, ngf * 2] => [batch, 64, 64, ngf * 4]
        ndf * 8,  # encoder_4: [batch, 64, 64, ngf * 4] => [batch, 32, 32, ngf * 8]
        ndf * 4,  # encoder_5: [batch, 32, 32, ngf * 8] => [batch, 16, 16, ngf * 16]
        ndf * 2,  # encoder_6: [batch, 16, 16, ngf * 16] => [batch, 8, 8, ngf * 16]
    ]
    n_layers = 5
    for i in range(n_layers):
        with tf.variable_scope("layer_%d" % (len(layers) + 1)):
            output = ResidualBlock(layers[-1], layers[-1].shape.as_list()[-1], layer_specs[i], 3,
                                   name='D.Block.%d' % (len(layers) + 1),
                                   spectral_normed=spectral_normed,
                                   update_collection=update_collection,
                                   inputs_norm=False,
    ...
    # layer_5: [batch, 32, 32, ndf * 2] => [batch, 32, 32, ndf]
    with tf.variable_scope("layer_%d" % (len(layers) + 1)):
        output = ResidualBlock(layers[-1], layers[-1].shape.as_list()[-1], ndf, 3,
                               name='D.Block.%d' % (len(layers) + 1),
                               spectral_normed=spectral_normed,
    ...
    # layer_6: [batch, 32, 32, ndf] => [batch, 32, 32, ndf]
    with tf.variable_scope("layer_%d" % (len(layers) + 1)):
        output = ResidualBlock(layers[-1], layers[-1].shape.as_list()[-1], ndf * 2, 3,
                               name='D.Block.%d' % (len(layers) + 1),
                               spectral_normed=spectral_normed,
...
def unet_generator_1(generator_inputs, generator_outputs_channels, ngf, conv_type, channel_multiplier, padding,
    ...
    for out_channels in layer_specs:
        with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
            ...
            # [batch, in_height, in_width, in_channels] => [batch, in_height/2, in_width/2, out_channels]
            convolved = lib.ops.conv2d.Conv2D(
                rectified, rectified.shape.as_list()[-1], out_channels, 5, 2, 'Conv2D',
                conv_type=conv_type, channel_multiplier=channel_multiplier,
                padding=padding, spectral_normed=True, update_collection=None,
                inputs_norm=False, he_init=True, biases=True)
    ...
    layer_specs = [
        ...
        (ngf * 16, 0.0),  # decoder_5: [batch, 8, 8, ngf * 8 * 2] => [batch, 16, 16, ngf * 8 * 2]
        (ngf * 8, 0.0),  # decoder_5: [batch, 16, 16, ngf * 8 * 2] => [batch, 32, 32, ngf * 8 * 2]
        (ngf * 4, 0.0),  # decoder_4: [batch, 32, 32, ngf * 8 * 2] => [batch, 64, 64, ngf * 4 * 2]
        (ngf * 2, 0.0),  # decoder_3: [batch, 64, 64, ngf * 4 * 2] => [batch, 128, 128, ngf * 2 * 2]
        (ngf, 0.0),  # decoder_2: [batch, 128, 128, ngf * 2 * 2] => [batch, 256, 256, ngf * 2]
    ...
    for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
        ...
        with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
            ...
            output = lib.ops.conv2d.Conv2D(
                resized_input, resized_input.shape.as_list()[-1], out_channels, 5, 1, 'Conv2D',
                conv_type=conv_type, channel_multiplier=channel_multiplier,
                padding=padding, spectral_normed=True, update_collection=None,
                inputs_norm=False, he_init=True, biases=True)
            # output = tf.layers.conv2d_transpose(rectified, out_channels, kernel_size=4, strides=(2, 2),
    ...
    with tf.variable_scope("decoder_1"):
        ...
        output = lib.ops.conv2d.Conv2D(
            resized_input, resized_input.shape.as_list()[-1], generator_outputs_channels, 5, 1, 'Conv2D',
            conv_type=conv_type, channel_multiplier=channel_multiplier,
            padding=padding, spectral_normed=True, update_collection=None,
            inputs_norm=False, he_init=True, biases=True)
        output = tf.nn.tanh(output)
...
def unet_discriminator_1(discrim_inputs, discrim_targets, ndf, spectral_normed, update_collection,
    ...
    # layer_1: [batch, 512, 512, in_channels * 2] => [batch, 256, 256, ndf]
    with tf.variable_scope("layer_1"):
        padded_input = tf.pad(inputs, [[0, 0], [2, 2], [2, 2], [0, 0]], mode="CONSTANT")
        convolved = lib.ops.conv2d.Conv2D(
    ...
    for i in range(n_layers):
        with tf.variable_scope("layer_%d" % (len(layers) + 1)):
            ...
            stride = 1 if i == n_layers - 1 else 2  # last layer here has stride 1
            padded_input = tf.pad(layers[-1], [[0, 0], [2, 2], [2, 2], [0, 0]], mode="CONSTANT")
            convolved = lib.ops.conv2d.Conv2D(
    ...
    with tf.variable_scope("layer_%d" % (len(layers) + 1)):
        padded_input = tf.pad(rectified, [[0, 0], [2, 2], [2, 2], [0, 0]], mode="CONSTANT")
        convolved = lib.ops.conv2d.Conv2D(
